...
export const AuthEditor: React.FC<AuthEditorProps> = ({ auth, onChange, hideInherit }) => {
  if (!auth) {
    return (
      <div className="flex flex-col items-center justify-center p-8 border border-dashed border-subtle rounded-xl bg-surface opacity-50">
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex flex-col gap-2">
        ...
        <div className="relative group">
          <select 
            ...
            className="w-full bg-surface border border-subtle rounded-lg px-4 py-3 text-[11px] font-bold text-main uppercase tracking-widest appearance-none focus:border-brand/50 outline-none transition-all cursor-pointer"
          >
      ...
      <div className="p-6 bg-surface border border-subtle rounded-xl space-y-6">
        {auth.type === 'none' && (
        ...
        {auth.type === 'bearer' && (
          <div className="space-y-3">
            ...
            <div className="relative">
              <Key size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--border-strong)]" />
              <input 
                type="text"
                value={auth.bearer || ''}
                onChange={(e) => onChange({ ...auth, bearer: e.target.value })}
                placeholder="BEARER_TOKEN..."
                className="w-full bg-elevated border border-subtle rounded-lg pl-10 pr-4 py-3 text-[11px] font-mono text-main placeholder:text-[var(--border-strong)] focus:border-brand/50 outline-none transition-all"
              />
            </div>
          </div>
        )}
        ...
        {auth.type === 'oauth2' && (
          <div className="space-y-3">
            ...
            <div className="relative">
              <Zap size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--border-strong)]" />
              <input 
                type="text"
            ...
            <div className="flex items-center gap-2 mt-2">
               ...
               <p className="text-[9px] text-[var(--border-strong)] uppercase italic">Advanced OAuth2 Handshake protocols coming soon...</p>
            </div>
          </div>
        )}
        ...
        {auth.type === 'basic' && (
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className="relative">
                <User size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--border-strong)]" />
                <input 
                  type="text"
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--border-strong)]" />
                <input 
                  type="password"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-elevated border border-subtle rounded-lg px-4 py-3 text-[11px] font-mono text-main placeholder:text-[var(--border-strong)] focus:border-brand/50 outline-none transition-all"
                />
              </div>
              <div className="space-y-2">
                ...
                {['header', 'query'].map((pos) => (
                  <button 
                    ...
                    className={cn(
                      ...
                      auth.apiKey?.addTo === pos ? "bg-brand text-main border-brand" : "border-subtle text-dim hover:border-strong"
                    )}
                  >
                    {pos}
                  </button>
