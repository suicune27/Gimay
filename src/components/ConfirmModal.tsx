...
export const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  const iconColor =
    variant === 'danger' ? 'text-red-500' : variant === 'warning' ? 'text-yellow-500' : 'text-[var(--brand)]';
  const bgColor =
    variant === 'danger' ? 'bg-red-500/10' : variant === 'warning' ? 'bg-yellow-500/10' : 'bg-[var(--brand)]/10';
  const confirmBtnClass =
    variant === 'danger'
      ...
      : variant === 'warning'
        ...
        : 'bg-[var(--brand)] hover:bg-[var(--brand-hover)] text-[var(--bg-deep)] shadow-[0_0_20px_rgba(var(--brand-rgb),0.15)]';
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative w-full max-w-md bg-input border border-subtle rounded-2xl shadow-2xl overflow-hidden"
      >
        {/* Header */}
        <div className="p-5 flex items-start gap-4">
          ...
          <button
            onClick={onClose}
            className="shrink-0 p-1 text-dim hover:text-white transition-colors"
          >
            <X size={16} />
          </button>
        </div>
