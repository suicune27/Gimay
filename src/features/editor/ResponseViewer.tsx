...
import { Clock, Database, Globe, Save, Terminal, Search, Copy, Check, Trash2 } from 'lucide-react';
import { PersistenceService } from '../../services/PersistenceService';
import Editor from '@monaco-editor/react';

export const ResponseViewer: React.FC = () => {
  const { lastResponse, setLastResponse } = useStore();
  ...
  if (!lastResponse) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-main bg-deep relative overflow-hidden">
        <motion.div
          animate={{ 
            scale: [1, 1.1, 1],
  ...
  return (
    <div className="h-full flex flex-col bg-deep">
      {/* Response Header */}
      <div className="h-9 px-4 border-b border-subtle flex items-center justify-between bg-surface shrink-0">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className={cn(
              "w-1.5 h-1.5 rounded-full",
              isError ? "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]" : "bg-brand shadow-[0_0_8px_rgba(62,207,142,0.4)]"
            )} />
            <span className={cn(
        ...
        <div className="flex items-center">
            <button 
              ...
              className="mr-3 px-2 py-1 flex items-center gap-1.5 text-[8px] font-black text-dim hover:text-brand transition-all uppercase tracking-widest hover:bg-white/5 rounded"
            >
              <Save size={10} />
              Snapshot
            </button>
            <div className="w-px h-3 bg-[var(--border-subtle)] mr-3" />
            <div className="flex">
              {(['Body', 'Headers', 'Tests', 'Console'] as const).map((tab) => (
                <button
                  ...
                  className={cn(
                    ...
                    activeTab === tab ? "border-brand text-brand" : "border-transparent text-dim hover:text-muted"
                  )}
                >
                  {tab}
                  {tab === 'Tests' && lastResponse.testResults && (
                    <span className={cn(
                      ...
                      lastResponse.testResults.every(r => r.status === 'pass') ? "bg-brand/20 text-brand" : "bg-red-500/20 text-red-500"
                    )}>
      ...
      <div className="flex-1 overflow-auto custom-scrollbar p-3">
        <div className="h-full w-full rounded-lg border border-subtle bg-surface overflow-hidden shadow-2xl">
          {activeTab === 'Body' && (
            <Editor
              height="100%"
          ...
          {activeTab === 'Tests' && (
            <div className="p-6 space-y-4">
               {lastResponse.testResults?.map((result, i) => (
                 <div key={i} className="flex items-center justify-between p-4 bg-surface rounded-xl border border-subtle">
                    <div className="flex items-center gap-3">
                       <div className={cn(
                         "w-1.5 h-1.5 rounded-full",
                         result.status === 'pass' ? "bg-brand" : "bg-red-500"
                       )} />
                       <div className="text-[11px] font-bold text-main uppercase tracking-tight">{result.name}</div>
                    </div>
                    <div className={cn(
                      "text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded",
                      result.status === 'pass' ? "bg-brand/10 text-brand" : "bg-red-500/10 text-red-500"
                    )}>
                      {result.status}
                    </div>
                 </div>
               ...
               {(!lastResponse.testResults || lastResponse.testResults.length === 0) && (
                 <div className="h-40 flex flex-col items-center justify-center text-main">
          ...
          {activeTab === 'Console' && (
            <div className="flex flex-col h-full bg-deep relative overflow-hidden">
               ...
               {/* Console Sub-header */}
               <div className="h-9 px-3 border-b border-[#1C1C1F] bg-surface/90 flex items-center justify-between shrink-0 z-20">
                 {/* Left side filters */}
                 <div className="flex items-center gap-1 bg-elevated border border-subtle p-0.5 rounded-lg shadow-inner">
                   {(['all', 'log', 'warn', 'error'] as const).map((lvl) => (
                     <button
                       ...
                       className={cn(
                         ...
                         consoleFilter === lvl 
                           ? "bg-brand/10 text-brand border border-brand/20 shadow-[0_0_8px_rgba(62,207,142,0.1)]" 
                 ...
                 <div className="flex items-center gap-2">
                   <div className="relative">
                     <Search size={9} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-dim" />
                     <input 
                       type="text"
                       placeholder="FILTER LOGS..."
                       value={consoleSearch}
                       onChange={(e) => setConsoleSearch(e.target.value)}
                       className="bg-surface border border-subtle rounded-lg pl-7 pr-2 py-0.5 text-[8px] font-mono text-main placeholder:text-dim focus:border-brand/40 focus:outline-none focus:ring-1 focus:ring-[#3ECF8E]/25 transition-all shadow-inner"
                     />
                   </div>
                    <button 
                      onClick={() => {
                      ...
                      className="p-1 text-muted hover:text-brand hover:bg-elevated disabled:opacity-30 disabled:hover:text-muted rounded-lg border border-transparent hover:border-strong transition-all"
                    ...
                    <button 
                      onClick={() => {
                        ...
                        useStore.getState().addToast({ type: 'info', message: 'Console logs cleared' });
                      }}
                      disabled={!lastResponse.consoleLogs || lastResponse.consoleLogs.length === 0}
                      className="p-1 text-muted hover:text-red-400 hover:bg-elevated disabled:opacity-30 disabled:hover:text-muted rounded-lg border border-transparent hover:border-strong transition-all"
               ...
               {/* Log Terminal List */}
               <div className="flex-1 overflow-y-auto font-mono text-[10px] selection:bg-brand/20 custom-scrollbar divide-y divide-[#131316] z-20">
                    {filteredConsoleLogs.map((log, i) => {
                      ...
                      return (
                        <div 
                          key={i} 
                          className={cn(
                            "flex gap-0 group items-stretch border-l-2 transition-all duration-150 relative hover:bg-elevated",
                        ...
                        >
                          {/* Segment ID */}
                          <div className="px-3.5 py-1.5 text-dim shrink-0 tabular-nums font-bold text-[8px] border-r border-subtle bg-surface/50 flex items-center justify-center min-w-[90px] select-none">
                          ...
                          <div className="flex-1 flex gap-3 px-3 py-1.5 items-start min-w-0 pr-12">
                            ...
                            <div className="flex-1 overflow-x-auto no-scrollbar pb-0.5">
                              <div className={cn(
                                ...
                                isErr ? 'text-red-300' :
                                isWarn ? 'text-yellow-200' : 'text-main'
                              )}>
                                {(log.args || []).map((arg: any, j: number) => (
                                  <div key={j} className="whitespace-pre-wrap break-all">
                                    {typeof arg === 'object' ? (
                                      <span className="text-brand/90 bg-brand/5 px-1 py-0.5 rounded border border-brand/10 select-all font-bold">
                          ...
                          <div className="absolute right-3 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-150">
                            <button
                              ...
                              title="Copy Row"
                            >
                              {copiedIndex === i ? <Check size={10} className="text-brand" /> : <Copy size={10} />}
                            </button>
                          </div>
                        </div>
                      );
                    ...
                    {filteredConsoleLogs.length === 0 && (
                      <div className="h-48 flex flex-col items-center justify-center relative py-12">
                        ...
                        <div className="w-9 h-9 rounded-xl bg-elevated border border-subtle flex items-center justify-center mb-3 shadow-[0_0_20px_rgba(0,0,0,0.8)] text-brand relative">
                          <Terminal size={16} className="animate-pulse opacity-30" />
                        </div>
                        <p className="text-[9px] font-black uppercase tracking-[0.4em] text-dim font-mono select-none">READY // NO PACKETS DETECTED</p>
                      </div>
                    )}
               </div>
            </div>
