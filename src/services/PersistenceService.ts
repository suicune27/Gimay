...
export class PersistenceService {
  ...
  static async saveHistory(entry: any) {
    const compactBody = (body: any) => {
      if (body === null || body === undefined) return undefined;
      if (typeof body === 'string') return body.slice(0, 2048);
      if (typeof body === 'object') {
        if (body.type && typeof body.type === 'string') {
          const content = typeof body.content === 'string' ? body.content.slice(0, 2048) : undefined;
          return { type: body.type, content };
        }
    ...
    const safeEntry = {
      ...
      request_data: entry?.request_data ? {
        id: entry.request_data.id,
        collection_id: entry.request_data.collection_id,
        folder_id: entry.request_data.folder_id,
        workspace_id: entry.request_data.workspace_id,
        user_id: entry.request_data.user_id,
        name: entry.request_data.name,
        type: entry.request_data.type,
        method: entry.request_data.method,
        url: entry.request_data.url,
        headers: Array.isArray(entry.request_data.headers) ? entry.request_data.headers.slice(0, 30) : [],
        params: Array.isArray(entry.request_data.params) ? entry.request_data.params.slice(0, 30) : [],
        bodyType: entry.request_data.bodyType,
        body: compactBody(entry.request_data.body),
        auth: entry.request_data.auth || { type: 'none' },
        created_at: entry.request_data.created_at,
        updated_at: entry.request_data.updated_at
      } : undefined,
      response_data: entry?.response_data ? {
        status: entry.response_data.status,
        statusText: entry.response_data.statusText,
        headers: entry.response_data.headers || {},
        time: entry.response_data.time,
        size: entry.response_data.size,
        body_preview: compactBody(entry.response_data.body)
      } : undefined
    };

    if (this.isOffline()) {
      const state = this.store.getState();
      state.setHistory([safeEntry, ...state.history].slice(0, 100));
      return;
    }
    ...
    if (error && String(error.message || '').toLowerCase().includes('column')) {
      const legacySafeEntry = { ...safeEntry };
      delete legacySafeEntry.workspace_id;
      delete legacySafeEntry.request_name;
      delete legacySafeEntry.request_data;
      delete legacySafeEntry.time;
      delete legacySafeEntry.size;
      
      const fallback = await supabase.from('history').insert([legacySafeEntry]);
      error = fallback.error;
    }
  ...
  static async createScriptLog(log: Partial<ScriptExecutionLog>) {
    if (this.isOffline()) return null;
    try {
      const compactLogs = (log.logs || []).slice(-150).map((l: any) => ({
        level: l?.level || 'info',
        message: typeof l?.message === 'string' ? l.message.slice(0, 2048) : String(l?.message || '').slice(0, 2048),
        timestamp: l?.timestamp || new Date().toISOString()
      }));

      const compactErrors = (log.errors || []).slice(-150).map((e: any) => ({
        level: e?.level || 'error',
        message: typeof e?.message === 'string' ? e.message.slice(0, 2048) : String(e?.message || '').slice(0, 2048),
      ...
      const safeLog = {
        ...
        variables_changed: log.variables_changed || {}
      };
